:mod:`xargparse` --- eXtendet argparse

.. module:: xargparse
:synopsis: Command-line option and argument parsing library using class based parsers.

.. moduleauthor:: Leon Vaiman <vleon1@gmail.com>

.. sectionauthor:: Leon Vaiman <vleon1@gmail.com>

Source code: :source:`src/xargparse.py`

Tutorial

This page contains the API reference information.

  The :mod:`xargparse` module makes it easy to write user-friendly command-line interfaces. The program defines what arguments it requires, and :mod:`xargparse` will figure out how to parse those out of :data:`sys.argv`. The :mod:`xargparse` module also automatically generates help and usage messages and issues errors when users give the program invalid arguments.
  Example
  ...
  The first step in using the :mod:`xargparse` is subclassing the :class:`ClassParser` class:

  >>> class Arguments(ClassParser):

  The :class:`ClassParser` subclass will hold all the information necessary to parse the command line into Python data types.
  Adding arguments

  Filling the :class:`ClassParser` subclass with information about program arguments is done by adding :Class:`Args` objects. Generally, these objects tell the :class:`ClassParser` subclass how to take the strings on the command line and turn them into objects. This information is stored and used when :meth:`~ClassParser.parse_args` is called. For example:
  ...
  Later, calling :meth:`~ClassParser.parse_args` will return an object with two attributes, integers and accumulate. The integers attribute will be a list of one or more ints, and the accumulate attribute will be either the :func:`sum` function, if --sum was specified at the command line, or the :func:`max` function if it was not.
  Parsing arguments

  :class:`ClassParser` parses arguments through the :meth:`~ClassParser.parse_args` method. This will inspect the command line, convert each argument to the appropriate type and then invoke the appropriate action. This will fill up the subclass with the parsed values:

  >>> Arguments().parse_args(['--sum', '7', '-1', '42'])
  Arguments(accumulate=<built-in function sum>, integers=[1, 2, 3])

    In a script, :meth:`~ClassParser.parse_args` will typically be called with no arguments, and the :class:`ClassParser` will automatically determine the command-line arguments from :data:`sys.argv`.
    ArgumentParser objects

    The following sections describe how each of these are used.
    _prog

    By default, :class:`ClassParser` objects use sys.argv[0] to determine how to display the name of the program in help messages. This default is almost always desirable because it will make the help messages match how the program was invoked on the command line. For example, consider a file named myprogram.py with the following code:

    from xargparse import *

    class Arguments(ClassParser):

      foo = Arg('--foo', help='foo help')
    ...
    To change this default behavior, another value can be supplied using the _prog= variable int the :class:`ClassParser` subclass:
    ...
    >>> class Arguments(ClassParser):
    ...   _prog = 'myprogram'
    ...   foo = Arg('--foo', help='foo of the %(prog)s program')
    ...
    >>> Arguments().print_help()
        ...
        outfile=<_io.TextIOWrapper name='output.txt' encoding='UTF-8'>)
          ...
          Namespace(infile=<_io.TextIOWrapper name='<stdin>' encoding='UTF-8'>,
            outfile=<_io.TextIOWrapper name='<stdout>' encoding='UTF-8'>)
              ...
              When :meth:`~ArgumentParser.add_argument` is called with action='store_const' or action='append_const'. These actions add the const value to one of the attributes of the object returned by :meth:`~ArgumentParser.parse_args`. See the action description for examples.
